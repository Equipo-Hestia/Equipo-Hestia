...
import {
  Calendar, Upload, FileSpreadsheet, Download, CheckCircle,
...
// Alias para auto-detección de columnas (normalizado, sin tildes)
...
function formatearFilas(
  ...
  columnas.forEach((col, idx) => {
    ...
    if (campo) fieldToIdx[campo] = idx
  })
  return filasRaw
    .map(row => ({
      email_docente: (row[fieldToIdx.email_docente] ?? '').trim(),
      codigo_asignatura: (row[fieldToIdx.codigo_asignatura] ?? '').trim(),
      seccion: (row[fieldToIdx.seccion] ?? '').trim(),
      semestre: (row[fieldToIdx.semestre] ?? '').trim(),
      sala: fieldToIdx.sala !== undefined
        ? (row[fieldToIdx.sala] ?? '').trim() || undefined : undefined,
      dia_semana: fieldToIdx.dia_semana !== undefined
        ? (row[fieldToIdx.dia_semana] ?? '').trim() || undefined : undefined,
      hora_inicio: fieldToIdx.hora_inicio !== undefined
        ? (row[fieldToIdx.hora_inicio] ?? '').trim() || undefined : undefined,
      hora_fin: fieldToIdx.hora_fin !== undefined
        ? (row[fieldToIdx.hora_fin] ?? '').trim() || undefined : undefined,
    }))
...
export function ImportarHorario() {
  ...
  const [columnasDetectadas, setCols] = useState<string[]>([])
  const [filasRaw, setFilasRaw] = useState<string[][]>([])
  const [mapeo, setMapeo] = useState<Record<string, string>>({})
  const [datosFormateados, setDatos] = useState<HorarioFila[]>([])
  ...
  async function leerArchivo(file: File) {
    try {
      // Importación dinámica de @sheetjs/xlsx (code-splitting, no carga hasta usar)
      const { read, utils } = await import('@sheetjs/xlsx')
      ...
      reader.onload = (e) => {
        ...
        if (!rows.length) { setErrorMsg('El archivo está vacío.'); return }
      ...
      setErrorMsg(
        'Error al leer el archivo. Asegúrate de que sea un Excel o CSV válido.'
      )
    }
  }
  ...
  function seleccionarArchivo(file: File) {
    ...
    if (!['csv', 'xlsx', 'xls'].includes(ext ?? '')) {
      setErrorMsg('Solo se aceptan archivos .csv, .xlsx o .xls'); return
    }
    setErrorMsg(null)
    leerArchivo(file)
  }
  ...
  function handleFormatear() {
    ...
    const datos = formatearFilas(filasRaw, columnasDetectadas, mapeo)
    if (!datos.length) {
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-0.5">
              Sube el Excel de DuocUC, mapea las columnas y carga el horario en Hestia.
            </p>
          </div>
        ...
        <div className="max-w-xl mx-auto">
          <div className="bg-teal-50 border border-teal-200 rounded-xl p-4 mb-5
            ...
            <p className="text-xs text-teal-700 leading-relaxed">
              Sube cualquier Excel de DuocUC. Hestia detectará las columnas
              automáticamente y te pedirá confirmar el mapeo antes de importar.
            </p>
          </div>
          {errorMsg && (
          ...
          <div
            ...
            <p className="font-semibold text-slate-700">
              Arrastra el Excel de DuocUC aquí
            </p>
            <p className="text-slate-400 text-sm mt-1">o haz clic para buscarlo</p>
            <p className="text-xs text-slate-300 mt-3">CSV, XLSX o XLS</p>
          </div>
        </div>
      )}
          ...
          <div className="lg:col-span-2 space-y-4">
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm">
              <div className="px-5 py-4 border-b border-slate-100">
                <p className="font-bold text-slate-900 text-sm">Columnas detectadas</p>
            ...
            <button onClick={handleFormatear}
              className="w-full flex items-center justify-center gap-2 bg-teal-600
                         hover:bg-teal-700 text-white font-bold py-3 rounded-xl
                         transition-colors text-sm">
              <ArrowRight size={15} />
              Formatear y previsualizar
            </button>
            ...
            {datosFormateados.length === 0 ? (
              <div className="bg-slate-50 border-2 border-dashed border-slate-200
                ...
                <p className="text-xs mt-1">
                  Configura el mapeo y presiona Formatear
                </p>
              </div>
            ) : (
              <>
                <div className="bg-white rounded-xl border border-slate-200
                  ...
                  <div className="px-5 py-3 border-b border-slate-100 bg-slate-50
                    ...
                    <p className="text-xs font-bold text-slate-600 uppercase tracking-wide">
                      Vista previa — {datosFormateados.length} filas
                    </p>
                  </div>
                  <div className="overflow-x-auto">
                    <table className="w-full text-xs">
                      <thead>
                        <tr className="border-b border-slate-100 bg-slate-50">
                          {['Email', 'Cód. Asig.', 'Secc.', 'Semestre', 'Día', 'Hora'].map(h => (
                            <th key={h}
                      ...
                      <tbody>
                        {datosFormateados.slice(0, 8).map((f, i) => (
                          <tr key={i}
                            className="border-b border-slate-50 hover:bg-slate-50">
                            <td className="px-3 py-2 text-slate-600 truncate max-w-32">
                              {f.email_docente}
                            </td>
                    ...
                    {datosFormateados.length > 8 && (
                      <p className="text-center text-xs text-slate-400 py-2">
                        ... y {datosFormateados.length - 8} filas más
                      </p>
                    )}
                  </div>
                </div>
                ...
                <div className="flex gap-3">
                  <button onClick={() => descargarCSV(datosFormateados)}
            ...
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-slate-500 text-sm">
                  Autoriza la importación de {datosFormateados.length} clases.
                </p>
              </div>
            </div>
            <div className="relative mb-4">
              <input
                type="text" inputMode="numeric" maxLength={6} value={codigoTotp}
                onChange={e => {
                  setTotp(e.target.value.replace(/\D/g, '')); setErrorMsg(null)
                }}
      ...
      {/* Cargando */}
      {paso === 'cargando' && (
        <div className="max-w-sm mx-auto bg-white rounded-2xl border border-slate-200
